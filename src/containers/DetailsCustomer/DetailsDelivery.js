...
console.log(this.state.customerdetails);
  ...
  return (
    ...
    <ul className = {classes.ul}>
        <li><h3 className={classes.h3}>משלוח אלי</h3></li>
        <li className = {classes.li}><input  placeholder= "הוסף שם פרטי" 
                                             className = {classes.form__field}
                                             onChange = {(event)=>this.editfirstname(event)}></input>
                                    </li>
        <li className = {classes.li}><input  placeholder= "הוסף שם משפחה"
                                             className = {classes.form__field}
                                             onChange = {(event)=>this.editlastname(event)}></input></li>
        <li className = {classes.li}><select>
                                            <option value="Null">בחר עיר</option>
                                            <option value="Ramle">רמלה</option>
                                            <option value="Rishon Letzion">ראשון לציון</option>
                                            <option value="Rehovot">רחובות</option></select></li>
        <li className = {classes.li}><input  placeholder= " הכנס רחוב"
                                             className = {classes.form__field}
                                             onChange = {(event)=>this.editaddress(event)}></input></li>
        <li className = {classes.li}><input  placeholder= " הכנס מספר רחוב"
                                             className = {classes.form__field}
                                             onChange = {(event)=>this.editnumberaddress(event)}></input></li>
        <li className = {classes.li}><input  placeholder= "הכנס דירה"
                                             className = {classes.form__field}
                                             onChange = {(event)=>this.editappartment(event)}></input></li>
        <li  className = {classes.li}><button className = {classes.Buttoncon}>אישור והמשך</button></li>
    </ul>
    </div>
    </motion.div>
  );
